...
export class InboxComponent {
  ...
  protected readonly filteredMessages = computed(() => {
    ...
    return this.messages().filter((m) => {
      ...
      if (q) {
        const haystack =
          `${m.senderName} ${m.subject} ${m.preview}`.toLowerCase();
  ...
  protected archive(id: string, event?: Event): void {
    ...
    this.messages.update((list) =>
      list.map((m) =>
        m.id === id ? { ...m, archived: true, starred: false } : m,
      ),
    );
    this.selected.update((set) => {
      const next = new Set(set);
  ...
  protected archiveSelected(): void {
    ...
    this.toast.success(`${ids.size} mensaje${ids.size === 1 ? '' : 's'} archivado${ids.size === 1 ? '' : 's'}`);
    this.clearSelection();
  }
  ...
  protected deleteSelected(): void {
    ...
    this.toast.error(`${ids.size} mensaje${ids.size === 1 ? '' : 's'} eliminado${ids.size === 1 ? '' : 's'}`);
    this.clearSelection();
  }
