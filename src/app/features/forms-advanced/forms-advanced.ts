...
export class FormsAdvancedComponent {
  ...
  protected readonly allTags: readonly Tag[] = [
    ...
    },
    {
      ...
      label: 'SSR',
      color:
        'bg-emerald-100 text-emerald-700 mm-dark:bg-emerald-900/40 mm-dark:text-emerald-300',
    },
    {
      id: 'design-system',
      label: 'Design System',
      color:
        'bg-fuchsia-100 text-fuchsia-700 mm-dark:bg-fuchsia-900/40 mm-dark:text-fuchsia-300',
    },
    {
      id: 'accessibility',
      label: 'Accesibilidad',
  ...
  protected addFiles(items: File[]): void {
    ...
    this.toast.success(`${next.length} archivo${next.length === 1 ? '' : 's'} agregado${next.length === 1 ? '' : 's'}`, {
      title: 'Listo',
    });
  }
