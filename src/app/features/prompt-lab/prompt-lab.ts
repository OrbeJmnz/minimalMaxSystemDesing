...
export class PromptLabComponent {
  ...
  protected readonly textareaRef =
    viewChild<ElementRef<HTMLTextAreaElement>>('promptArea');
...
protected restoreFromHistory(entry: HistoryEntry): void {
  ...
  protected generate(): void {
    ...
    this.clearStream();
    this.streamFullText = this.buildResponse(
      this.promptText(),
    ...
    this.streamTimer = window.setInterval(() => {
      ...
      this.streamIndex = Math.min(
        this.streamIndex + step,
        this.streamFullText.length,
      );
      this.streamedText.set(this.streamFullText.slice(0, this.streamIndex));
    }, STREAM_INTERVAL_MS);
  }
