...
@Component({
  ...
  template: `
    @if (open()) {
      <div class="fixed inset-0 z-50 mm-no-print" role="dialog" aria-modal="true" [attr.aria-labelledby]="titleId">
        <button
          type="button"
          (click)="close.emit()"
          aria-label="Cerrar"
        ...
        >
          ...
          >
            <div class="flex-1 min-w-0">
              ...
              <h3
                [id]="titleId"
                class="font-display text-lg font-semibold text-ink-dark truncate"
              >
                {{ title() }}
              </h3>
            </div>
