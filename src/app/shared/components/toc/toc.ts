...
@Component({
  ...
  template: `
    @if (items().length > 1) {
      <aside class="hidden lg:block w-56 shrink-0 mm-no-print sticky top-24 self-start max-h-[calc(100dvh-7rem)] overflow-y-auto mm-scroll-overlay">
...
export class TocComponent implements AfterViewInit {
  ...
  private scan(): void {
    ...
    const targets = Array.from(
      document.querySelectorAll<HTMLElement>('[data-mm-toc-id]'),
    );
    ...
    const same =
      ...
      current.every((it, i) => it.id === items[i]?.id);
