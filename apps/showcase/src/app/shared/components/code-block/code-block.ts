...
export class CodeBlockComponent {
  ...
  private highlightTs(code: string): string {
    ...
    escaped = escaped.replace(/\b(\d+\.?\d*)\b/g, '<span class="mm-tk-number">$1</span>');
    ...
    escaped = escaped.replace(/\b([a-zA-Z_$][\w$]*)(?=\s*\()/g, '<span class="mm-tk-fn">$1</span>');
    return escaped;
  }

  private highlightHtml(code: string): string {
    let escaped = escapeHtml(code);
    escaped = escaped.replace(/(&lt;!--[\s\S]*?--&gt;)/g, '<span class="mm-tk-comment">$1</span>');
    escaped = escaped.replace(/(&lt;\/?)([\w-]+)/g, '$1<span class="mm-tk-tag">$2</span>');
    escaped = escaped.replace(/\s([\w-]+)=/g, ' <span class="mm-tk-attr">$1</span>=');
    escaped = escaped.replace(/=(&quot;[^&]*?&quot;)/g, '=<span class="mm-tk-string">$1</span>');
    return escaped;
  }
  ...
  private highlightCss(code: string): string {
    let escaped = escapeHtml(code);
    escaped = escaped.replace(/(\/\*[\s\S]*?\*\/)/g, '<span class="mm-tk-comment">$1</span>');
    escaped = escaped.replace(/([\w-]+)(?=\s*:)/g, '<span class="mm-tk-property">$1</span>');
    escaped = escaped.replace(
      /(&quot;[^&]*?&quot;|&#39;[^&]*?&#39;)/g,
      '<span class="mm-tk-string">$1</span>',
    );
  ...
  private highlightJson(code: string): string {
    ...
    escaped = escaped.replace(/\b(\d+\.?\d*)\b/g, '<span class="mm-tk-number">$1</span>');
    escaped = escaped.replace(/\b(true|false|null)\b/g, '<span class="mm-tk-keyword">$1</span>');
    return escaped;
  }
}
